...
class TextBox:
    def __init__(self, x, y, width, height, player_id=None):
        ...
        self.active = False
        self.player_id = player_id  # Add a player ID associated with this TextBox

    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Toggle the active state of the textbox
        ...
        if event.type == pygame.KEYDOWN:
            if self.active:
                if event.key == pygame.K_RETURN:
                    # Save or update the player codename in the database
                    if self.player_id:
                        update_player_name(self.player_id, self.text)
                        print(f"Player {self.player_id}'s codename updated to {self.text}")
                    else:
                        # If no player ID, create a new player
                        player_id = random.randint(1000, 9999)
                        insert_player(player_id, self.text)
                        self.player_id = player_id
                        print(f"Added new player: {self.text} with ID: {player_id}")
...
for row in range(10):
    table_row = []
    player_id = random.randint(1000, 9999)  # Create a new player ID for each row
    for col in range(2):
        x = 100 + col * cell_width
        y = 50 + row * cell_height
        table_row.append(TextBox(x, y, cell_width, cell_height, player_id))
...
while running:
    for event in pygame.event.get():
        ...
        # Handle mouse and keyboard events for text boxes
        for table in tables:
            for row in table:
                for text_box in row:
        ...
        # Handle keypress events for buttons
        elif event.type == pygame.KEYDOWN:
            if event.key in key_to_action:
                key_to_action[event.key]()

    # Update the screen based on the state (splash screen or main screen)
    if on_splash_screen:
        ...
        # Draw the main screen with tables and buttons
        screen.fill((255, 255, 255))
        # Draw header and other UI elements
        BLACK = (0, 0, 0)
        WHITE = (255, 255, 255)
...
# Close game resources when ending
end_game()
