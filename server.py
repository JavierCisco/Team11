...
class Server():
    ...
    def start(self):
        MESSAGE = '[LISTENING] Server is listening on {RECEIVE_PORT}'
        ...
        while True:
            ...
            if data:
                MESSAGE = '[RECEIVED] Data from {addr}: {data.decode(FORMAT)}'
    ...
    def start_traffic(self):
        ...
        MESSAGE = '[BROADCASTING] Starting game with code 202'
    ...
    def stop(self):
        ...
        MESSAGE = '[BROADCASTING] Stopping game with code 221'
        for _ in range(3):
    ...
    def handle_client(self, msg: str):
        MESSAGE = '[DEBUG] Received message: {msg}'
        if ':' in msg:
            parts = msg.split(':')
            if len(parts) == 2:
                transmit_id, hit_id = parts
                MESSAGE = '[HIT EVENT] Transmit ID: {transmit_id}, Hit ID: {hit_id}'
                self.send_hit_id(transmit_id, hit_id)
            else:
                MESSAGE = '[ERROR] Malformed message. Expected "player_id:hit_id", got: {msg}'
        elif msg.isdigit():
            MESSAGE = '[SINGLE ID RECEIVED] Message: {msg}'
            # Handle single ID messages if needed
            self.server_broadcast.sendto(msg.encode(FORMAT), BROADCAST_ADDR)
        else:
            MESSAGE = '[ERROR] Unrecognized message format: {msg}'
    ...
    def send_hit_id(self, equip_id_str: str, hit_id_str: str):
        ...
        if hit_id == 43:  # Green Base hit
            if equip_id % 2 == 0:
                MESSAGE = '[GREEN BASE HIT] Friendly fire by {equip_id}'
            else:
                points = 100
                message = f'{hit_id}'
        elif hit_id == 53:  # Red Base hit
            if equip_id % 2 != 0:
                MESSAGE = '[RED BASE HIT] Friendly fire by {equip_id}'
            else:
                points = 100
                message = f'{hit_id}'
        ...
        if message:
            MESSAGE = '[BROADCASTING] Sending message: {message}'
    ...
    def update_points(self, equip_id: int, hit_id: int, points: int):
        ...
        MESSAGE = '[POINTS UPDATE] Equip ID: {equip_id}, Hit ID: {hit_id}, Points: {points}'
